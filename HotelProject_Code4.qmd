...
### Important! Perfect Multicollinearity

::: {.callout-important title="Handling Perfect Multicollinearity in Our Logistic Regression Model"}
During the development of our logistic regression model, we encountered unexpected results in the initial model summary. Almost all predictors showed extremely high p-values, and many coefficient estimates were unstable or close to zero. Despite this, the predictive performance appeared artificially strong at first, which prompted a deeper investigation. The unusual combination of non-significant coefficients and suspiciously high predictive accuracy suggested a structural issue in the data rather than a modelling error. This led us to suspect the presence of perfect multicollinearity.

To verify this, we calculated the Variance Inflation Factor (VIF) for all predictors. VIF is a standard diagnostic tool used to identify whether a predictor is highly correlated with other predictors in the model. In a well-specified model, VIF values should generally remain below 5–10. However, our analysis showed that several variables produced extremely high VIF scores, in some cases far beyond acceptable thresholds. This confirmed that certain predictors were linearly dependent on one another, meaning the model could not uniquely estimate their individual effects.

A closer inspection revealed the source of the problem: many of the date- and time-related variables were strongly correlated by definition. For example, `arrival_date_month`, `arrival_date_day_of_month`, `arrival_date_week_number`, and `arrival_date_year` all contain overlapping temporal information and therefore explain essentially the same underlying structure. Additionally, reservation_status_date and assigned_room_type contributed further redundancy. When multiple variables encode the same information in slightly different forms, the model struggles to separate their effects, which leads to perfect or near-perfect multicollinearity.
:::
...
### Select Randomly 20% of Data for Faster Rendering
```{r}
set.seed(12345)
data <- data %>%
  sample_frac(0.2)
...
```{r}
data_corr <- data_log %>%
  select(y,lead_time,is_repeated_guest,stays_in_weekend_nights,customer_type)
data_corr
```

```{r}
df_numeric <- data_corr[sapply(data_corr, is.numeric)]
...
data_ann <- data_ann %>%
  select(-distribution_channelUndefined,-reserved_room_typeL, -reserved_room_typeP)
...
#train_first_knn <- train_first_knn %>%
  #select(-distribution_channelUndefined)

test_first_knn <- data_knn[-train_rows,]
#test_first_knn <- test_first_knn %>%
  #select(-distribution_channelUndefined)
...
```{r}
train_first_knn_x <- train_first_knn[,-59]
test_first_knn_x <- test_first_knn[,-59]
train_first_knn_y <- train_first_knn[,59]
test_first_knn_y <- test_first_knn[,59]
...
knn_prob_kappa <- ifelse(
  best_knn_pred == "1",
  attr(best_knn_pred, "prob"),
  1 - attr(best_knn_pred, "prob")
)
```
...
knn_prob_profit <- ifelse(
  best_knn_profit_pred == "1",
  attr(best_knn_profit_pred, "prob"),
  1 - attr(best_knn_profit_pred, "prob"))
```
...
```{r}
model_ann_base <- neuralnet(y ~ ., data = train_first_ann)
plot(model_ann_base)
```

```{r}
can_pred_ann <- predict(model_ann_base, test_first_ann)
can_bin_pred <- ifelse(can_pred_ann >= 0.5, 1, 0)
ANNmodel <- confusionMatrix(as.factor(can_bin_pred), as.factor(test_first_ann$y), positive = "1")
ANNmodel
```
...
```{r}
# Ergebnistabelle ohne Threshold-Spalte
...
# verschiedene ANN-Architekturen
hidden_layers <- list(
  c(6, 4)
  # z.B. weitere Modelle:
  # c(4),
...
# Zielvariable + Testfeatures
train_first_ann$y <- factor(train_first_ann$y, levels = c(0, 1))
test_first_ann$y  <- factor(test_first_ann$y,  levels = c(0, 1))
x_test <- subset(test_first_ann, select = -y)
y_test <- test_first_ann$y

for (h in hidden_layers) {
  
  # eindeutiger Modellname, z.B. "ann_6-4"
  model_id <- paste0("ann_", paste(h, collapse = "-"))
  ...
  # 2) Wahrscheinlichkeiten einmal berechnen
  pred_raw <- compute(nn_fit, x_test)$net.result
  
  prob_test <- if (is.matrix(pred_raw) && ncol(pred_raw) == 2) {
    as.vector(pred_raw[, 2])     # P(y = 1), falls zwei Ausgänge
  } else {
    as.vector(pred_raw)          # sonst direkt
  }
  
  # optional: Probs für diese ANN-Version speichern
  assign(paste0("ann_prob_", model_id), prob_test)
  
  # 3) Klassifikation mit fixem Threshold 0.5
  pred_num  <- ifelse(prob_test >= 0.5, 1, 0)
  pred_fact <- factor(pred_num, levels = c(0, 1))
  
  cm <- confusionMatrix(pred_fact, y_test, positive = "1")
  
  acc   <- as.numeric(cm$overall["Accuracy"])
  sens  <- as.numeric(cm$byClass["Sensitivity"])
  spec  <- as.numeric(cm$byClass["Specificity"])
  kappa <- as.numeric(cm$overall["Kappa"])
  gain  <- gain_function(cm)
...
```
```{r}
# Sicherstellen: Zielvariable als Faktor mit Levels 0/1
train_first_svm$y <- factor(train_first_svm$y, levels = c(0, 1))
test_first_svm$y  <- factor(test_first_svm$y,  levels = c(0, 1))

# -------------------------------
# 1. SVM-Modelle trainieren
# -------------------------------

set.seed(123)

svm_model_rbfdot <- ksvm(
  y ~ .,
  data       = train_first_svm,
  kernel     = "rbfdot",
  prob.model = TRUE
)

svm_model_polydot <- ksvm(
  y ~ .,
  data       = train_first_svm,
  kernel     = "polydot",
  prob.model = TRUE
...
# -------------------------------
# 2. Threshold-Schleife + Ergebnisse
# -------------------------------
...
# Hilfsfunktion: Wahrscheinlichkeiten für y = 1 ausgeben
get_prob_pos <- function(model, newdata) {
  prob_mat <- predict(model, newdata, type = "probabilities")
  # Annahme: zweite Spalte = Klasse "1"
  as.vector(prob_mat[, 2])
}

# Alle Kernel in einer Liste managen
svm_models <- list(
  rbfdot    = svm_model_rbfdot,
  polydot   = svm_model_polydot,
  vanilladot = svm_model_vanilladot
)

# Liste zum Speichern der Wahrscheinlichkeiten pro Kernel (einmal berechnet)
svm_prob_list <- list()

for (k in names(svm_models)) {
  
  model <- svm_models[[k]]
  
  # 1) Wahrscheinlichkeiten für das Testset EINMAL berechnen
  prob_pos <- get_prob_pos(model, test_first_svm)
  svm_prob_list[[k]] <- prob_pos   # hier speicherst du die Probabilities für jedes Modell
  
  # 2) Über Thresholds iterieren
  for (t in thresholds) {
    
    pred_num  <- ifelse(prob_pos >= t, 1, 0)
    pred_fact <- factor(pred_num, levels = c(0, 1))
    
    cm <- confusionMatrix(pred_fact, test_first_svm$y, positive = "1")
...
# -------------------------------
# 3. Standard-Threshold 0.5 auswerten
# -------------------------------

svm_standard_results <- subset(svm_results, threshold == 0.5)
svm_standard_results

# -------------------------------
# 4. Beste Modelle nach Gain & Kappa
# -------------------------------

# Global bestes Ergebnis nach Gain/Kappa
svm_maxgain_results  <- svm_results[which.max(svm_results$gain), ]
svm_maxkappa_results <- svm_results[which.max(svm_results$kappa), ]

svm_maxgain  <- max(svm_results$gain)
svm_maxkappa <- max(svm_results$kappa)

svm_maxgain_results
svm_maxkappa_results

# -------------------------------
# 5. Probabilities für die besten Modelle
# -------------------------------

# Bester Kernel nach Gain
best_kernel_gain   <- svm_maxgain_results$model
best_thresh_gain   <- svm_maxgain_results$threshold
best_prob_gain     <- svm_prob_list[[best_kernel_gain]]

# Bester Kernel nach Kappa
best_kernel_kappa  <- svm_maxkappa_results$model
best_thresh_kappa  <- svm_maxkappa_results$threshold
best_prob_kappa    <- svm_prob_list[[best_kernel_kappa]]

# Beispiel: Vorhersagen für bestes Gain-Modell mit dem optimalen Threshold
best_pred_gain <- factor(
  ifelse(best_prob_gain >= best_thresh_gain, 1, 0),
  levels = c(0, 1)
)

cm_best_gain <- confusionMatrix(best_pred_gain, test_first_svm$y, positive = "1")
cm_best_gain
